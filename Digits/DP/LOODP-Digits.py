...
def train(model, criterion, optimizer, dataloader, epoch, print_every, privacy_engine, device=device):
    losses = []
    model.train()
...
def training_only(model, criterion, optimizer, dataloader, epochs, privacy_engine):
    print("Train stats: \n")
    for epoch in range(epochs):
        PRINT_EVERY = epoch % 99 == 0
        train(model, criterion, optimizer, dataloader, epoch, PRINT_EVERY, privacy_engine)
...
training_only(model, criterion, optimizer, dataloader, EPOCHS, privacy_engine)
...
for idx in range(len(X_train)):
    ...
    privacy_engine_copy = PrivacyEngine(secure_mode=SECURE_MODE)
    ...
    model_copy, optimizer_copy, dataloader = privacy_engine_copy.make_private_with_epsilon(
        module=model_copy,
    ...
    training_only(model_copy, criterion, optimizer_copy, dataloader, EPOCHS, privacy_engine_copy)
...
for i in range(1,50):
  ...
  training_only(model = model_copy, criterion = criterion, optimizer = optimizer_copy, dataloader = dataloader, epochs = EPOCHS, privacy_engine = None)
