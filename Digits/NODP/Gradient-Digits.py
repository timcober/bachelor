...
for epoch in range(EPOCHS):
  ...
  for batch, target in trainloader:
    ...
    batch_grads.extend(torch.cat([elem.flatten(1) for elem in per_sample_grads], 1).norm(2, 1).cpu().numpy().tolist())
  ...
  if epoch!=0 and epoch%5==0:
    print(f"Epoch {epoch+1}: Loss {epoch_losses[epoch+1][0]:.6f}, Train accuracy {epoch_accs[epoch+1][0]:.6f}")
